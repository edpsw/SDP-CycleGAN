...
class preprocess:
    ...
    def create_df(self, df_train, df_test):
        ...
        to_drop_U2R = [0, 1, 2, 3]
        to_drop_Dos = [0, 1, 4, 3]
        to_drop_Probe = [0, 1, 2, 4]
        to_drop_Attack = [0]
        to_drop_none =[]

        train_normal = df_train[~df_train['attack_type'].isin(to_drop_normal)] # normal_df中只包含训练集中的normal，为的是在使用SMOTE时更方便
        train_R2L = df_train[~df_train['attack_type'].isin(to_drop_R2L)] # R2L_df是GAN的输入，只包含R2L攻击
        train_U2R = df_train[~df_train['attack_type'].isin(to_drop_U2R)]
        train_Dos = df_train[~df_train['attack_type'].isin(to_drop_Dos)]
        train_Probe = df_train[~df_train['attack_type'].isin(to_drop_Probe)]
        train_attack = df_train[~df_train['attack_type'].isin(to_drop_Attack)]
        df_train = df_train[~df_train['attack_type'].isin(to_drop_none)]
        
        
        test_normal = df_test[~df_test['attack_type'].isin(to_drop_normal)] 
        test_R2L = df_test[~df_test['attack_type'].isin(to_drop_R2L)] 
        test_U2R = df_test[~df_test['attack_type'].isin(to_drop_U2R)]
        test_Dos = df_test[~df_test['attack_type'].isin(to_drop_Dos)]
        test_Probe = df_test[~df_test['attack_type'].isin(to_drop_Probe)]
        test_attack = df_test[~df_test['attack_type'].isin(to_drop_Attack)]
        df_test = df_test[~df_test['attack_type'].isin(to_drop_none)]


        return df_train, df_test, train_normal, train_R2L, train_U2R, train_Dos, train_Probe,test_normal, test_R2L, test_U2R, test_Dos, test_Probe,train_attack,test_attack
